...
st.markdown("""
    <style>
    .stApp {
        background: linear-gradient(135deg, #050810 0%, #0D1826 100%);
    }
    </style>
...
/* Closed selectbox - multiple selector attempts for compatibility */
...
#you can use LLM models like openai or lamma or deepseek or any other open ai models
...
Have questions or need assistance? Reach out at sanishadere5@gmail.com or connect with me on [LinkedIn](https://linkedin.com/in/your-linkedin-handle).

- 📧 **Email**: sanishadere5@gmail.com
- 💼 **LinkedIn**: Sanisha Dere [ www.linkedin.com/in/sanisha-dere-8abb703b1 ]
- 💻 **GitHub**: [View source code](https://github.com/your-username/your-repo-name)
...
if(st.button("Predict") and test_image is not None):
    with st.spinner("Please wait..."):
        ...
        #os.remove(temp_file_path) or
        class_name = ['CNV', 'DME', 'DRUSEN', 'NORMAL']

    st.success("Model is Prediction it's a {}".format(class_name[result_index]))
    ...
    with st.expander("Learn More"):
        ...
        if(result_index==0):
            ...
            ''')
            st.image(test_image)
            # so after adding LLM model you can import your cnv or insted importing cnv 
            # you can write here prompt thet "generate recommendation for this desease" write like this -
            # cnv = "generate recommendation for Choroidal Neovascularization (CNV)"
            # responce = llm()
            # BY THIS YOU CAN INTEGRATE GENAI ALSO
            st.markdown(cnv)
        ...
        #Normal
            ...
            st.write('''
            OCT scan showing *Normal retina with preserved foveal contour.*
            ''')
            st.image(test_image)
            st.markdown(normal)
    #deleting tempfile
